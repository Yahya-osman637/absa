...
function printOrder(orderId = null) {
	if(orderId) {		
        ...
        mywindow.document.write('<html><head><title>Carwo Canab</title>');
        mywindow.document.write('</head>ZAAD:402947 / E-DAHAB:29465 /  CAll-Number:0633721394<body>');
	...
	$.ajax({
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'</td>'+
				'<td style="padding-left:20px;"">'+
					'<input type="text" name="cost[]" id="cost'+count+'" autocomplete="off" disabled="true" class="form-control" />'+
					'<input type="hidden" name="cost[]" id="cost'+count+'" autocomplete="off" class="form-control" />'+
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
					'<input type="number" name="quantity[]" id="quantity'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" min="1" />'+
					'<input type="number" name="quantity[]" id="quantity'+count+'" onkeyup="getcost('+count+')" autocomplete="off" class="form-control" min="1" />'+
					'</div>'+
				'</td>'+
				'<td style="padding-left:20px;">'+
				...
				'</td>'+
				'<td style="padding-left:20px;">'+
					'<input type="text" name="t_cost[]" id="t_cost'+count+'" autocomplete="off" class="form-control" disabled="true" />'+
					'<input type="hidden" name="t_cost[]" id="t_cost'+count+'" autocomplete="off" class="form-control" />'+
				'</td>'+
				'<td>'+
...
function getProductData(row = null) {
	if(row) {
		...
		if(productId == "") {
			...
			$("#cost"+row).val("");
			$("#quantity"+row).val("");						
			$("#total"+row).val("")=$("#rate"+row).val("") * $("#quantity"+row).val("");
			$("#t_cost"+row).val("")=$("#cost"+row).val("") * $("#quantity"+row).val("");
			...
			$.ajax({
				...
				success:function(response) {
					...
					var t_cost = Number(response.cost) * 1;
					t_cost = t_cost.toFixed(2);
					$("#t_cost"+row).val(t_cost);
					$("#t_cost"+row).val(t_cost);
...
function getTotal(row = null) {
	if(row) {
		...
		var t_cost = Number($("#cost"+row).val()) * Number($("#quantity"+row).val());
		total = total.toFixed(2);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total);
		subAmount();
...
function subAmount() {
	...
	for(x = 0; x < tableProductLength; x++) {
		...
		// t_cost =  Number(t_cost) + Number($("#t_cost"+count).val());
...
function vat() {
	var tableProductLength = $("#productTable tbody tr").length;
	var total=0
	for(y = 0; y < tableProductLength; y++) {
		var tr = $("#productTable tbody tr")[y];
		var count = $(tr).attr('id');
		count = count.substring(3);

		total = Number(total) + Number($("#t_cost"+count).val());
		// t_cost =  Number(t_cost) + Number($("#t_cost"+count).val());
